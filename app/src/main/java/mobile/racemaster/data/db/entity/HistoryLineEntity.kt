...
data class HistoryLineEntity(
    ...
    // Null for a RETIRE row (never crosses any timing point), a CP-mode PASS row (a real
    // checkpoint crossing, but CP's own checkpoint is never paired with a Time Mode device the
    // way Bibs Mode's finish line is — see EntryLogModeEngine.NO_SPLIT_ACTIONS), or a Time Mode
    // STOP row (see TimeModeRepository.stopStopwatch's own doc). None of these get a
    // splitNumber nor consume the shared counter, so the very next real crossing still gets
    // the number it would otherwise have gotten. Every other action, including markers like
    // START/RESET/CLOCK, still gets one.
    val splitNumber: Int?,
