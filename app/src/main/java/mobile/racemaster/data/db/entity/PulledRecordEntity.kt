...
data class PulledRecordEntity(
    ...
    // wasting BLE airtime/battery long after every confirmation has already landed. This is a
    // separate concern from syncedAtMillis itself: that field means "is this confirmed at all",
    // this one means "have I already told the device that gave me this record about that
    // confirmation" — orthogonal, both nullable, set at different times. Only ever set once
    // PeripheralSyncService's own GATT write-response for that ack is deferred until its
    // markSynced call genuinely finishes (see that class's own doc), so "the write succeeded"
    // is now a trustworthy signal that the confirmation was actually durably relayed, not just
